...
export async function getSummaryStats(): Promise<SummaryStats> {
  ...
  // Government data is published daily; if the newest record we have is more
  // than 2 days old, the daily cron likely failed silently — flag it rather
  // than showing visitors quietly outdated prices.
  let is_stale = false;
  if (row.latest_date) {
    const latest = new Date(row.latest_date);
